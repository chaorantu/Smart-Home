...
static uint16 EndDeviceID = 0x0005 ; //终端ID，重要
//---------------------------------------------------------------------
...
#define DATA_PIN05   P0_5    //定义P0.5口为光敏的输入端
...
uint8 NodeData[MAX_NODE][6];         //终端数据缓冲区 0=温度 1=湿度 2=气体 3=灯
uint8 TxBuffer[128];
...
static uint8 GetSecurity( void );
static uint8 XorCheckSum(uint8 * pBuf, uint8 len);
static uint16 ReadLightData( void );
  ...
  case SERIALAPP_CLUSTERID:
    ...
    case 0x3A:	
      ...
      {  
        ...
        NodeData[afRxData[2]-1][4] = afRxData[9];
        NodeData[afRxData[2]-1][5] = 0x00;
        state0=afRxData[8];
        sprintf(str, "BRI:%03d ", afRxData[7]);
        gas=afRxData[6];
        secustate=afRxData[9];
        wendu[0]=afRxData[4]/10+0x30;
        wendu[1]=afRxData[4]%10+0x30;
        shidu[0]=afRxData[5]/10+0x30;
        shidu[1]=afRxData[5]%10+0x30;
        LCD_write_EN_string(45,80,wendu);
        LCD_write_EN_string(45,95,shidu);
        LCD_write_CN_string(60,80,"℃");
        LCD_write_CN_string(60,95,"％");
        LCD_write_EN_string(70,110,str);
        if(gas==0x00)
        {
          LCD_write_CN_string(9,110,"气体异常");
        }
        else
        {
          LCD_write_CN_string(9,110,"气体正常");
        }
        if(secustate==0x00)
        {
          LCD_write_EN_string(70,80,"security");
        }
        else
        {
          LCD_write_EN_string(70,80,"insecurity");
        }
      }
    ...
    case 0x3A:  //开关灯设备          
      ...
      else if(afRxData[3] == 0x08)    //电机 功能码为08
      {
        if(EndDeviceID == afRxData[2] || afRxData[2] == 0xFF)//电机 EndDeviceID为06
        {
         ...
         else if(afRxData[3] == 0x09)      //继电器 功能码为09
      {
        if(EndDeviceID == afRxData[2] || afRxData[2] == 0xFF)  //继电器 EndDeviceID为05
        {
          if(afRxData[4] == 0)
          {
            LAMP_PIN=1;
...
uint8 SendData(uint8 addr, uint8 FC)
{
  ...
  case 0x01: //查询所有终端传感器的数据
    ...
    {
      osal_memcpy(&TxBuffer[index], NodeData[i], 5);
      index += 5;
    }
      ...
      (SerialApp_TxLen = HalUARTRead(UART0, SerialApp_TxBuf, SERIAL_APP_TX_MAX)))
  {
    if (SerialApp_TxLen)
    {
      ...
      if(SerialApp_TxBuf[0] == 0x3A)
      {
        ...
        {
          if(FC == 7 || FC == 8 || FC == 0x0A || FC == 0x0B || FC == 0x0C || FC == 9) //控制终端
          {                            
...
void SerialApp_SendPeriodicMessage( void )
{
  ...
  SendBuf[6] = GetGas();  //获取气体传感器的状态 
   if(DATA_PIN05 == 1)
    state0 = 0x30;       //当光敏电阻处于黑暗中时P0.5高电平
  else
    state0 = 0x31;
  SendBuf[7]=ReadLightData();
  SendBuf[8]=state0;//获得光敏电阻的状态
  SendBuf[9]=GetSecurity();
  SendBuf[10] = XorCheckSum(SendBuf, 11);
  SendBuf[11] = 0x23;
