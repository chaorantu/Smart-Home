...
UINT CSerialPort::CommThread(LPVOID pParam)
{
	...
	{ 
		...
		OutputDebugString("WaitForMultipleObjects.\r\n");
		...
		case 1:	// read event
			{
				OutputDebugString("ReceiveChar begin.\r\n");
				GetCommMask(port->m_hComm, &CommEvent);
				if (CommEvent & EV_RXCHAR) //接收到字符，并置于输入缓冲区中 
					ReceiveChar(port, comstat);

				OutputDebugString("ReceiveChar end.\r\n");
#if 0		
		...
		case 2: // write event
			{
				OutputDebugString("WriteChar begin.\r\n");
				// Write character event from port
				WriteChar(port);
				OutputDebugString("WriteChar end.\r\n");
				break;
			}
...
void CSerialPort::ReceiveChar(CSerialPort* port, COMSTAT comstat)
{
	...
	for (;;) 
	{ 
		OutputDebugString("ReceiveChar wait 11\r\n");
		...
		if(WaitForSingleObject(port->m_hShutdownEvent,0)==WAIT_OBJECT_0)  
			...
			OutputDebugString("ReceiveChar wait 22\r\n");
		...
		sprintf(printf_buff, "read count=%d\r\n", comstat.cbInQue);
		OutputDebugString(printf_buff);
		...
		sprintf(printf_buff, "bRead=%d\r\n", bRead);
		OutputDebugString(printf_buff);

		if (bRead)
		{
			sprintf(printf_buff, "ReadFile\r\n");
		...
		sprintf(printf_buff, "bRead111=%d\r\n", bRead);
		OutputDebugString(printf_buff);
